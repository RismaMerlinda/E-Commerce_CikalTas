                        ...
                        <div class="order-header">
                            ...
                            <div style="text-align: right;">
                                @if ($order->status == 'pending')
                                    ...
                                    <div class="action-buttons" style="margin-top: 16px;">
                                        <form action="{{ route('admin.orders.mark-paid', $order) }}" method="POST">
                                            @csrf
                                            <button type="submit" class="btn btn-brown btn-small"
                                                onclick="return confirm('Tandai pesanan ini sebagai sudah dibayar?')">
                                                Tandai Lunas
                                            </button>
                                        </form>
                                    </div>
                                @elseif($order->status == 'paid' || $order->status == 'processing')
                                    ...
                                    <div class="action-buttons" style="margin-top: 16px;">
                                        <form action="{{ route('admin.orders.mark-completed', $order) }}"
                                            method="POST">
                                            @csrf
                                            <button type="submit" class="btn btn-brown btn-small"
                                ...
                                @else
                                    <span class="status-badge status-failed">{{ ucfirst($order->status) }}</span>
                                @endif
                            </div>
                        </div>
                    </div>
